...
//script for block upadte
function updateblockstatusonfb(id,msg){
//  alert("hk");
var pathbl="blockupdate/"+id+"/"
var rrt=adminid+msg;
database.ref(pathbl).set({
  action : adminid
...
var starCountRef2 = firebase.database().ref('blockupdate/');
starCountRef2.on('value', function(snapshot) {

  snapshot.forEach((child) => {
//  if(child.key==adminid){

  //}
if(child.key==adminid){
  if(child.val().action!='no'){
//call loader
exceptionhandlerblock(child.val().action)
var pathlr="blockupdate/"+adminid+"/";
database.ref(pathlr).set({
  action : 'no'
